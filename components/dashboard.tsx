...
import { Plus, Download, CalendarCheck, Search, LogOut } from "lucide-react"
...
import { hasVisitExactlyToday } from "@/lib/visit-utils"
...
export function Dashboard({ onLogout }: DashboardProps) {
  ...
  }, [subjects, siteFilter, staffFilter, searchQuery, exactTodayMode])
        ...
        <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-2 px-4 py-2.5 sm:gap-3">
          ...
          <div className="flex items-center gap-2 rounded-md border border-border bg-card px-2.5 py-1">
            ...
            <Switch
              ...
              onCheckedChange={setExactTodayMode}
              className="scale-75"
            />
          </div>
        ...
        {filteredSubjects.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <p className="text-sm text-muted-foreground">
              {exactTodayMode
                ? "오늘 당일 방문 예정 환자가 없습니다. / No patients expected today."
                : subjects.length === 0
